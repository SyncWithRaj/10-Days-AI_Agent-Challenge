...
import {
  ...
  Unlock, Globe, Database, Fingerprint, ShieldAlert, Mic, Volume2, Loader2, HeartPulse, Brain, List, CheckSquare, Ear
} from "lucide-react";
...
// --- COMPONENT: ADVANCED VITALS MONITOR ---
const VitalsMonitor = ({ data }) => {
  ...
  const ProgressBar = ({ value, max, color }) => {
    ...
    return (
        <div className="progress-container">
            <div className="progress-fill" style={{ width: `${width}%`, backgroundColor: color, boxShadow: `0 0 10px ${color}` }}></div>
        </div>
    );
  };
  ...
  return (
    <div className="hex-panel">
      <div className="panel-header"><HeartPulse size={12} /> BIO_TELEMETRY</div>
      <div className="order-content">
        
        {/* MOOD SCORE */}
        <div className="metric-row">
            <div className="metric-label">MOOD_INDEX [{bio.mood_score || 0}/10]</div>
            <ProgressBar value={bio.mood_score || 0} max={10} color="#00f3ff" />
        </div>

        {/* STRESS LOAD */}
        <div className="metric-row">
            <div className="metric-label">STRESS_LOAD [{bio.stress_load || 0}%]</div>
            <ProgressBar value={bio.stress_load || 0} max={100} color="#ff0055" />
        </div>

        <div className="order-row">
            <span className="field-label">ENERGY_STATE:</span>
            <span className="field-value cyan">{bio.energy_level || "SCANNING..."}</span>
        </div>

        <div className="order-row">
            <span className="field-label">SLEEP_DATA:</span>
            <span className="field-value">{bio.sleep_status || "UNKNOWN"}</span>
        </div>

        {/* DIRECTIVES */}
        <div className="order-row extras">
          <span className="field-label"><List size={10}/> DIRECTIVES:</span>
          <div className="goals-list">
            {dirs.length > 0 ? (
                dirs.map((d, i) => (
                    <div key={i} className="goal-item">
                        <span className={`prio ${d.priority === 'HIGH' ? 'red' : 'green'}`}>[{d.priority?.substring(0,1) || '-'}]</span> {d.task.toUpperCase()}
                    </div>
                ))
            ) : (<span className="field-value">AWAITING_INPUT</span>)}
          </div>
        </div>
        ...
        {data.analysis?.recommendation && (
            <div className="rec-box">
                {/* ✅ FIXED: Wrapped arrows in quotes */}
                <span className="rec-label">{">>"} PROTOCOL:</span>
                <span className="rec-text">{data.analysis.recommendation.toUpperCase()}</span>
            </div>
        )}
...
export default function Home() {
  ...
  // 🧬 ADVANCED VITALS STATE
  const [vitalsState, setVitalsState] = useState({
    biometrics: { mood_score: 0, stress_load: 0, energy_level: "", sleep_status: "" },
    directives: [],
  ...
  const startWakeWordListener = () => {
    ...
    if (!SpeechRecognition) { alert("Wake word not supported."); handleStartSession(); return; }
    ...
    recognition.onresult = (event) => {
        ...
        if (transcript.includes("nexus") || transcript.includes("hello") || transcript.includes("start")) {
            recognition.stop(); setIsStandby(false); handleStartSession();
        }
    };
    recognition.onerror = (e) => console.log("Wake error", e);
    try { recognition.start(); wakeWordRecognition.current = recognition; setIsStandby(true); } 
    catch (e) { handleStartSession(); }
  };
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        if (blob.size > 0) {
          setMessages(p => [...p, { type: 'info', text: ">> ANALYZING_BIO_SIGNALS..." }]);
  ...
  const processAudioPipeline = async (blob) => {
    ...
    try {
      ...
      const genRes = await fetch("/api/generate", {
        ...
        body: JSON.stringify({ text: transData.transcript, currentVitals: vitalsState }),
      });
  ...
  const handleSystemInit = () => startWakeWordListener();
  const handleStartSession = () => {
    if (wakeWordRecognition.current) wakeWordRecognition.current.stop();
    setIsStandby(false);
    setIsSessionActive(true);
    sessionActiveRef.current = true;
    shouldTerminateRef.current = false;
    setVitalsState({ biometrics: {}, directives: [], analysis: {} });
    setMessages([{ type: 'system', text: "SYNCING NEURAL INTERFACE..." }]);
    startRecording();
  };
    ...
    <main className="crt-wrapper">
      <div className="screen-layer">
        ...
        <div className="top-bar">
          <div className="bar-group"><Terminal size={14} className="icon-pulse" /> <span className="term-title">NEXUS // BIO-SYNC v4.0</span></div>
          <div className="bar-group center">
            {isSessionActive ? <span className="secure">VITAL_LINK_ACTIVE</span> : isStandby ? <span className="standby">STANDBY_MODE</span> : <span className="offline">OFFLINE</span>}
        ...
        <div className="main-grid">
          <div className="side-panel left">
            <div className="panel-box">
              <div className="panel-header"><Brain size={12} /> NEURAL_LOAD</div>
              <div className="graph-container">
                {cpuGraphData.map((h, i) => <div key={i} className="bar" style={{ height: `${h}%`, opacity: i < 8 ? 1 : 0.3 }}></div>)}
              </div>
              <div className="stat-row"><span>CAPACITY</span><span className="val">{ramUsage}%</span></div>
            </div>
            <div className="panel-box globe-box">
              <div className="panel-header"><Globe size={12} /> SUBJECT_LOC</div>
              <div className="wireframe-globe"></div>
              <div className="geo-text">SECURE_SECTOR_09</div>
            </div>
          </div>
 ...
 \\_| \\_/\\____/ \\/   \\/ \\___/ \\____/ 
`}
                  <div className="intro-text"><br />{isStandby ? <div className="blink-yellow"><DecryptText text=">> LISTENING FOR 'HEY NEXUS'..." /></div> : <DecryptText text=">> SYSTEM_OFFLINE. INITIALIZE." />}</div>
                </div>
              )}
              {messages.map((msg, i) => (
          ...
          <div className="side-panel right">
            ...
            <VitalsMonitor data={vitalsState} />
          </div>
        </div>

        <div className="control-deck">
          <div className="deck-wing left"><div className="wing-label"><Mic size={12} /> INPUT</div><FrequencyWing active={recording} color="#00f3ff" reverse={true} /></div>
          <div className="deck-center">
            {!isSessionActive && !isStandby && (<button className="main-button start" onClick={handleSystemInit}><div className="btn-content"><HeartPulse size={24} /><span>CHECK_IN</span></div><div className="btn-ring"></div></button>)}
            {isStandby && (<div className="standby-indicator"><Ear size={32} className="icon-pulse" color="#ffaa00" /><span>LISTENING...</span></div>)}
            {isSessionActive && (<div className="active-controls"><button className={`main-button active ${recording ? 'recording' : ''}`} onClick={stopRecording} disabled={!recording}><div className="btn-content">{recording ? <><div className="rec-dot"></div><span>LISTENING</span></> : <><Loader2 size={24} className="spin" /><span>ANALYZING</span></>}</div><div className="btn-ring"></div></button><button className="abort-btn" onClick={handleEndSession}><ShieldAlert size={16} /> STOP</button></div>)}
          </div>
          <div className="deck-wing right"><div className="wing-label"><Volume2 size={12} /> OUTPUT</div><FrequencyWing active={isSpeaking} color="#bd00ff" reverse={false} /></div>
        </div>
      </div>
      ...
      <style jsx global>{`
        ...
        :root { --g: #00ff41; --d: #0d0208; --dim: #003b00; --err: #ff0033; --cyan: #00ffff; --purp: #bd00ff; --yel: #ffaa00; }
        ...
        .secure { color: var(--g); } .offline { color: var(--err); } .standby { color: var(--yel); animation: blink 1s infinite; }
        ...
        .progress-container { width: 100%; height: 6px; background: #222; border: 1px solid #444; }
        ...
        .goal-item { font-size: 10px; color: var(--cyan); border-left: 2px solid var(--g); padding-left: 5px; margin-top: 2px; }
        .prio.red { color: var(--err); } .prio.green { color: var(--g); }
        .rec-box { border: 1px solid var(--purp); padding: 5px; margin-top: 10px; background: rgba(189,0,255,0.1); }
        .rec-label { color: var(--purp); font-size: 10px; display: block; margin-bottom: 2px; } .rec-text { color: #fff; font-size: 11px; }
        ...
        .standby-indicator { display: flex; flex-direction: column; align-items: center; color: var(--yel); font-size: 12px; font-weight: bold; gap: 10px; }
        ...
        .blink-yellow { animation: blink 1s infinite; color: var(--yel); }
