...
import { 
  ...
  Unlock, Globe, Database, Fingerprint, ShieldAlert, Mic, Volume2, Loader2
} from "lucide-react";
...
const MemoryDump = () => {
  ...
  useEffect(() => {
    const initialLines = Array(12).fill(0).map(() => `0x${generateHex(4)}: ..`);
    setLines(initialLines);
    const interval = setInterval(() => {
      setLines(prev => [...prev.slice(1), `0x${generateHex(4)}: ${generateHex(2)} ${generateHex(2)}`]);
    }, 100);
    return () => clearInterval(interval);
  }, []);
  return (
    <div className="hex-panel">
      <div className="panel-header"><Database size={10}/> MEM_BUFFER</div>
      <div className="hex-content">
        {lines.map((line, i) => <div key={i} className="hex-line">{line}</div>)}
      </div>
    </div>
  );
};
...
const HackerTypewriter = ({ text }) => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      i++;
      if (i > text.length) { clearInterval(timer); setDisplay(text); }
    }, 15); 
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setNetSpeed({ up: (Math.random() * 2).toFixed(1), down: (Math.random() * 10).toFixed(1) });
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        if (blob.size > 0) {
            setMessages(p => [...p, { type: 'info', text: ">> ENCRYPTING_PACKET..." }]);
  ...
  const processAudioPipeline = async (blob) => {
    ...
    try {
      ...
      const genRes = await fetch("/api/generate", {
        ...
        body: JSON.stringify({ text: transData.transcript }),
      });
      if (!sessionActiveRef.current) return;
  ...
  const handleStartSession = () => {
    ...
    setMessages([{ type: 'system', text: "INITIALIZING N.E.X.U.S. v9.0..." }]);
    startRecording();
  };
      ...
      <div className="screen-layer">
        ...
        <div className="top-bar">
          <div className="bar-group">
            <Terminal size={14} className="icon-pulse" /> <span className="term-title">ROOT@SMARTY:~</span>
          </div>
          <div className="bar-group center">
            {isSessionActive ? <span className="secure">SECURE_CONNECTION</span> : <span className="offline">OFFLINE</span>}
        ...
        <div className="main-grid">
            ...
            <div className="side-panel left">
                <div className="panel-box">
                    <div className="panel-header"><Cpu size={12}/> CORE_01</div>
                    <div className="graph-container">
                        {cpuGraphData.map((height, i) => (
                            <div key={i} className="bar" style={{ height: `${height}%`, opacity: i < 8 ? 1 : 0.3 }}></div>
                        ))}
                    </div>
                    <div className="stat-row"><span>LOAD</span><span className="val">{ramUsage}%</span></div>
                </div>
                <div className="panel-box globe-box">
                    <div className="panel-header"><Globe size={12}/> TARGET</div>
                    <div className="wireframe-globe"></div>
                    <div className="geo-text">LOC: 34.2, -118.4</div>
                </div>
            </div>
            ...
            <div className="terminal-window">
                <div className="terminal-header">
                    ...
                    <div className="tab active">/bin/nexus_core</div>
                </div>
...
{`
 ...
 | |\\  || |___  / /^\\ \\| |_| |/\\__/ /
`}
                        <div className="intro-text"><br/><DecryptText text=">> SYSTEM_READY_FOR_INJECTION..." /></div>
                       </div>
                    )}
                    {messages.map((msg, i) => (
                        <div key={i} className={`log-line ${msg.type}`}>
                            <span className="prompt">{msg.type === 'user' ? 'root@smarty:~$ ' : msg.type === 'ai' ? 'nexus@ai:~$ ' : 'kernel:~$ '}</span> 
            ...
            <div className="side-panel right">
                <div className="panel-box">
                    <div className="panel-header"><Fingerprint size={12}/> KEYS</div>
                    <div className="auth-key">RSA: <span className="blur">A9F...</span></div>
                </div>
                <MemoryDump />
            </div>
        </div>
                ...
                {!isSessionActive ? (
                    ...
                    <div className="active-controls">
                        ...
                        <button className="abort-btn" onClick={handleEndSession}>
                            <ShieldAlert size={16} /> ABORT
                        </button>
                    </div>
                )}
            </div>

            {/* Right Wing: Agent Audio */}
      ...
      <style jsx global>{`
        ...
        /* RIGHT PANEL */
        .hex-panel { height: 100%; display: flex; flex-direction: column; }
        .hex-content { font-size: 10px; color: #444; font-family: monospace; flex: 1; overflow: hidden; }
        .hex-line { margin-bottom: 2px; }
