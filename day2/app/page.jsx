...
import {
  ...
  Unlock, Globe, Database, Fingerprint, ShieldAlert, Mic, Volume2, Loader2, Coffee, CheckCircle
} from "lucide-react";
...
// --- COMPONENT: LIVE ORDER MANIFEST (Updated Memory Dump) ---
const OrderManifest = ({ order }) => {
  const fields = [
    { key: 'drinkType', label: 'BEVERAGE_ID' },
    { key: 'size', label: 'VOLUME_CLASS' },
    { key: 'milk', label: 'LIQUID_BASE' },
    { key: 'name', label: 'ENTITY_NAME' },
  ];

  return (
    <div className="hex-panel">
      <div className="panel-header"><Coffee size={12} /> LIVE_ORDER_MANIFEST</div>
      <div className="order-content">
        {fields.map((f) => (
          <div key={f.key} className={`order-row ${order[f.key] ? 'filled' : 'empty'}`}>
            <span className="field-label">{f.label}:</span>
            <span className="field-value">
              {order[f.key] ? order[f.key].toUpperCase() : "NULL"}
              {order[f.key] && <CheckCircle size={10} className="check-icon" />}
            </span>
          </div>
        ))}
        <div className="order-row extras">
          <span className="field-label">ADD_ONS:</span>
          <span className="field-value">
            {order.extras && order.extras.length > 0 ? order.extras.join(", ").toUpperCase() : "NONE"}
          </span>
        </div>
      </div>
      <div className="hex-footer">
        <span>STATUS: {order.name && order.drinkType ? "PROCESSING" : "INCOMPLETE"}</span>
      </div>
    </div>
  );
};

// --- COMPONENT: TYPEWRITER (FIXED) ---
...
export default function Home() {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        if (blob.size > 0) {
          setMessages(p => [...p, { type: 'info', text: ">> ANALYZING_VOICE_PATTERN..." }]);
  ...
  const processAudioPipeline = async (blob) => {
    ...
    try {
      ...
      // ✅ PASS CURRENT ORDER STATE TO API
      ...
      // ✅ UPDATE ORDER STATE FROM AI
      if (genData.order) {
        setOrderState(genData.order);
      }
      ...
      if (genData.isComplete) {
        shouldTerminateRef.current = true; // End call after completion msg
      }
  ...
  const handleStartSession = () => {
    ...
    // Reset Order
    setOrderState({ drinkType: "", size: "", milk: "", extras: [], name: "" });
    setMessages([{ type: 'system', text: "BOOTING BARISTA_PROTOCOL v2.0..." }]);
    startRecording();
  };
      ...
      <div className="screen-layer">
        ...
        <div className="top-bar">
          <div className="bar-group">
            <Terminal size={14} className="icon-pulse" /> <span className="term-title">ROOT@SMARTY_CAFE:~</span>
          </div>
        ...
        <div className="main-grid">
          ...
          <div className="side-panel left">
            <div className="panel-box">
              <div className="panel-header"><Cpu size={12} /> KITCHEN_LOAD</div>
              <div className="graph-container">
                {cpuGraphData.map((height, i) => (
                  <div key={i} className="bar" style={{ height: `${height}%`, opacity: i < 8 ? 1 : 0.3 }}></div>
                ))}
              </div>
              <div className="stat-row"><span>BUSY_RATE</span><span className="val">{ramUsage}%</span></div>
            </div>
            <div className="panel-box globe-box">
              <div className="panel-header"><Globe size={12} /> LOCATION</div>
              <div className="wireframe-globe"></div>
              <div className="geo-text">NEXUS_BREWS_HQ<br />SECTOR_7G</div>
            </div>
          </div>
          ...
          <div className="terminal-window">
            <div className="terminal-header">
              ...
              <div className="tab active">/bin/barista_bot</div>
            </div>
...
\\_| \\_/\\____/ \\/   \\/ \\___/ \\____/ 
`}
                  <div className="intro-text"><br /><DecryptText text=">> WAITING_FOR_CUSTOMER..." /></div>
                </div>
              )}
              {messages.map((msg, i) => (
                <div key={i} className={`log-line ${msg.type}`}>
                  <span className="prompt">{msg.type === 'user' ? 'cust@smarty:~$ ' : msg.type === 'ai' ? 'barista@ai:~$ ' : 'sys:~$ '}</span>
          ...
          {/* --- RIGHT SIDEBAR (ORDER MANIFEST) --- */}
          <div className="side-panel right">
            <div className="panel-box">
              <div className="panel-header"><Fingerprint size={12} /> ORDER_ID</div>
              <div className="auth-key">ID: <span className="blur">#8X9-22</span></div>
            </div>

            {/* ✅ LIVE ORDER MANIFEST */}
            <OrderManifest order={orderState} />
          </div>
        </div>

        {/* --- BOTTOM CONTROL DECK (CENTERED) --- */}
        <div className="control-deck">
          <div className="deck-wing left">
            <div className="wing-label"><Mic size={12} /> INPUT_FREQ</div>
          ...
          <div className="deck-wing right">
            <div className="wing-label"><Volume2 size={12} /> OUTPUT_FREQ</div>
